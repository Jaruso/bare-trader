...
- README: added Prerequisites (Python, pipx) with Mac and Windows notes; MCP section rewritten so Claude Desktop uses `"command": "baretrader", "args": ["mcp", "serve"]` with no wrapper script; documented config file locations for macOS and Windows; added Troubleshooting
